...
module adc_adaq4001_spi_min #(
  ...
  assign spi_mosi = (state > 0) ? output_reg[C_ADC_BIT_WIDTH-1] : 1;  // Shift out command/data during transaction, MSB first
    ...
    if (rst) begin
      ...
      case (state)
        0: begin
          ...
          adc_ready_reg <= 0;
          spi_clk <= 0; // Ensure SPI clock is low during conversion
          // default command to read ADC data and preserve default settings 
           // (see ADAQ4001 datasheet, rev B, Table 14 & Figure 53)
          output_reg <= 16'h1400;
        ...
        1: begin
          state <= 2;
          adc_data_reg <= 0; // Clear ADC data register before new transaction
        end
        2: begin // SPI transaction: shift out command and read data, run SPI clock
          if (clock_counter < C_ADC_BIT_WIDTH) begin
            ...
            if (spi_clk == 1) begin
              // On falling edge, shift out command and read MISO
              output_reg <= {output_reg[C_ADC_BIT_WIDTH-2:0], 1'b0}; // Shift out command bits, MSB first
            ...
            spi_clk <= 0; // Ensure SPI clock is low after transaction
          end
        end
      endcase
    end
