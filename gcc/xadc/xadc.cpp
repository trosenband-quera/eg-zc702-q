...
int main(int argc, char *argv[]) {
    ...
    unsigned ms = 5;
    std::vector<unsigned> channels_to_read = {0,1,2,3,4,5,6}; // default: all channels

    // Channel names and offsets
    const char* channel_names[] = {"TEMP", "VCCINT", "VCCAUX", "VP/VN", "VREFP", "VREFN", "VCCBRAM"};
    const unsigned channel_offsets[] = {0x00, 0x04, 0x08, 0x0C, 0x10, 0x14, 0x18};
    ...
    const unsigned bipolar[8] = {0,     0,        0,       1,       0,        1,    0, 0};
    ...
    while (n < nmax) {
        ...
        if (!quiet && showscaled) {
            ...
            for (size_t i = 0; i < raw_values.size(); ++i) {
                ...
                float val = low_scaled * scale[channels_to_read[i]] / 4096.0 + (channels_to_read[i] == 0 ? Toff : 0);
